...
<Accordion>
  ...
  <Accordion.Item eventKey="9">
    ...
    <Accordion.Body>
      ...
      > - **Scalability:** Rootstock can achieve a higher transaction throughput than Liquid because Rootstock's transactions are smaller, its blocks can contain more transactions, and the Rootstock blockchain has a lower average block interval. Rootstock has implemented parallel transaction execution ([RSKIP-144](https://github.com/rsksmart/RSKIPs/blob/master/IPs/RSKIP144.md)), which allows transactions to be processed simultaneously on multiple threads.
    </Accordion.Body>
  </Accordion.Item>
...
<Accordion>
  ...
  <Accordion.Item eventKey="2">
   ...
   <Accordion.Item eventKey="3">
    ...
    <Accordion.Body>
      - The block gas limit and the average block rate determine the number of transactions per second executable on the Rootstock platform. The current average block rate is one block every 30 seconds. The miner can vote to increase the block gas limit at each mined block. Currently, the block gas limit is 6.8M gas units per block, this  value can change for when Parallel Transaction Executions are enabled. A simple rBTC transaction consumes 21K gas, so the Rootstock platform can execute 11 transactions per second today.
      - Rootstock has implemented parallel transaction execution ([RSKIP-144](https://github.com/rsksmart/RSKIPs/blob/master/IPs/RSKIP144.md)), which partitions transactions into parallel sublists that can be executed simultaneously on multiple threads. This allows increasing the block gas limit without proportionally increasing block execution time. With parallel execution enabled, for each new parallel thread, there is an increase of gas limit to be used by each additional parallel thread. It's around 700K gas per additional thread present. Currently, the plan is that we will have an additional thread and the block gas limit is increased from 6.8M to 7.5M gas units. The gas limit is distributed across parallel execution threads, with each thread receiving its own gas allocation to process transactions concurrently.
    </Accordion.Body>
  </Accordion.Item>
  <Accordion.Item eventKey="4">
