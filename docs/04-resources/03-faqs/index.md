  ...
  <Accordion.Item eventKey="2">
   ...
   <Accordion.Item eventKey="3">
    ...
    <Accordion.Body>
      ...
      - Rootstock has implemented parallel transaction execution ([RSKIP-144](https://github.com/rsksmart/RSKIPs/blob/master/IPs/RSKIP144.md)), which partitions transactions into parallel sublists that can be executed simultaneously on multiple threads. This allows increasing the block gas limit without proportionally increasing block execution time. With parallel execution enabled, the block gas limit is increased from 6.8M to 7.5M gas units. The gas limit is distributed across parallel execution threads, with each thread receiving its own gas allocation to process transactions concurrently.
    </Accordion.Body>
  </Accordion.Item>
  <Accordion.Item eventKey="4">
