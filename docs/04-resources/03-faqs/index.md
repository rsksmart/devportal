...
<Accordion>
  ...
  <Accordion.Item eventKey="9">
    ...
    <Accordion.Body>
      ...
      > - **Scalability:** Rootstock can achieve a higher transaction throughput than Liquid because Rootstock's transactions are smaller, its blocks can contain more transactions, and the Rootstock blockchain has a lower average block interval. Rootstock has implemented parallel transaction execution ([RSKIP-144](https://github.com/rsksmart/RSKIPs/blob/master/IPs/RSKIP144.md)), which allows transactions to be processed simultaneously on multiple CPU cores, significantly improving scalability and throughput.
    </Accordion.Body>
  </Accordion.Item>
  ...
  <Accordion.Item eventKey="11">
    ...
    <Accordion.Body>
      ...
      - Rootstock requires much fewer resources than Ethereum regarding blockchain size and state size. This is due to less on-chain activity and the fact that Rootstock uses more efficient data structures to manage the state, such as the Unitrie. The Unitrie enables parallel transaction execution ([RSKIP-144](https://github.com/rsksmart/RSKIPs/blob/master/IPs/RSKIP144.md)), which allows transactions to be processed simultaneously on multiple CPU cores, achieving higher transaction throughput and improved scalability.
    </Accordion.Body>
  </Accordion.Item>
</Accordion>
````
...
<Accordion>
  ...
  <Accordion.Item eventKey="2">
   ...
   <Accordion.Item eventKey="3">
    ...
    <Accordion.Body>
      - The block gas limit and the average block rate determine the number of transactions per second executable on the Rootstock platform. The current average block rate is one block every 30 seconds. The miner can vote to increase the block gas limit at each mined block. Currently, the block gas limit is 6.8M gas units per block. A simple rBTC transaction consumes 21K gas, so the Rootstock platform can execute 11 transactions per second today.
      - Rootstock has implemented parallel transaction execution ([RSKIP-144](https://github.com/rsksmart/RSKIPs/blob/master/IPs/RSKIP144.md)), which partitions transactions into parallel sublists that can be executed simultaneously on multiple CPU cores. This allows increasing the block gas limit without proportionally increasing block execution time, significantly improving transaction throughput and scalability. Miners create execution plans that partition transactions based on their storage access patterns, while full nodes execute these partitioned transactions in parallel.
    </Accordion.Body>
  </Accordion.Item>
  <Accordion.Item eventKey="4">
